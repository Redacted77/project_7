...
import os, tempfile
import base64
from . import errors_exceptions as err
from . import logs_system as log
import sqlite3 as sql
from . import database_manager as db_manger
...
class SercuritySystem():
    def __init__(self, logging_manger: log.Logging, db_path: Path = None,
                  encrypted_db: Path = None, salt: Path = None):
        
        data_path = Path.cwd() / "app_data"
        data_path.mkdir(exist_ok=True)

        self.db_path = db_path if db_path is not None else data_path / "face_locking_system.db"
        self.encrypted_db_path = encrypted_db if encrypted_db is not None else data_path / "FolderLockApp_System.db"
        self.salt = salt if salt is not None else data_path / "imp_salt.salt"
